...
Trains DenseNet121+BERT and EfficientNet+BERT models using image + text data
...
from models import MultimodalDenseNet, MultimodalEfficientNet
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(os.path.join(LOGS_DIR, "multimodal_models_training.log")),
...
class MultimodalModelTrainer:
    """Trainer for multimodal models (image + text)"""
    ...
    def create_model(self):
        ...
        if self.model_name == "multimodal_densenet":
        ...
        elif self.model_name == "multimodal_efficientnet":
    ...
    def create_optimizer(self, model):
        """Create optimizer and scheduler with different learning rates for different components"""
        ...
        optimizer = optim.AdamW([
            {'params': image_params, 'lr': TRAINING_CONFIG['learning_rate']},
            {'params': text_params, 'lr': TRAINING_CONFIG['learning_rate'] * 0.1},  # Lower LR for pretrained BERT
            {'params': fusion_params, 'lr': TRAINING_CONFIG['learning_rate']}
...
def train_multimodal_models():
    """Train both multimodal models"""
    print("🏥 Medical Image Classification - Step 3: Train Multimodal Models")
    print("=" * 70)
    ...
    for model_name in ['multimodal_densenet', 'multimodal_efficientnet']:
        if MODEL_CONFIGS[model_name]:
            print(f"\n🚀 Training {model_name}...")
            
            trainer = MultimodalModelTrainer(model_name, device)
...
def main():
    ...
    parser.add_argument('--model', type=str, choices=['multimodal_densenet', 'multimodal_efficientnet', 'both'], 
    ...
    if args.model == 'both':
        ...
        trainer = MultimodalModelTrainer(args.model, device)
        model = trainer.train(train_loader, val_loader)
